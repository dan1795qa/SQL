...
-- Для того, чтобы выделить номер месяца из даты используется функция MONTH(date_first)
-- Вывести информацию о командировках, начало и конец которых относятся к одному месяцу (год может быть любой). В результат включить столбцы name, city, date_first, date_last. 
...
--- Для того, чтобы выделить название месяца из даты используется функция MONTH(date_first)
...
-- Для GROUP BY можно указать (работает) ИНОГДА НА НЕКОТОРЫХ ПЛАТФОРМАХ!!!!!!!!!:
* вычисляемое выражение;
* исходное имя столбца;
* имя результирующего столбца, заданное через AS;
